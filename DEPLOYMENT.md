...
- Env vars: `DEEPGRAM_API_KEY`, `RELAY_AUTH_SECRET` (must match the Vercel app's `RELAY_AUTH_SECRET` below — the relay now refuses to start without it, and refuses live connections that don't present a valid token signed with it)
...
| `RELAY_AUTH_SECRET` | same value as the relay's `RELAY_AUTH_SECRET` on Render |
...
`relay/server.js` now scopes display broadcasts per talk show (keyed by `talkShowId`), and live connections require a short-lived token minted by `/api/relay/token` for that specific talk show — so testing locally against the deployed relay while someone else is live no longer cross-contaminates displays, and a stranger can't open a live connection against the relay directly.
